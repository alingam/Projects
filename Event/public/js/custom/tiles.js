...
(function($) {
    ...
    Tile.prototype.resize = function(cellRect, pixelRect, animate, duration, onComplete) {
        ...
        function vendorPrefixes(item,prop,value){
            var browserArr = ['-webkit-','-moz-','-o-','-ms-',''];
            for(var i = 0; i < browserArr.length; i++){
                item.$el.css(browserArr[i]+prop,value);
        ...
        //-webkit-transform: translate3d(605px, 5px, 0px);
        if(Modernizr.csstransitions) {

            vendorPrefixes(this,'transform',translate(cssChanges.left, cssChanges.top));
        }
        ...
        var tile = this,
            validateChangesAndComplete = function() {
                ...
                if(Modernizr.csstransitions) {
                    vendorPrefixes(tile,'transform',translate(tile.left, tile.top));
                }
        ...
        if (animate && changed) {
            ...
            if(!Modernizr.csstransitions) {
                ...
                this.$el.css({height:cssChanges.height, width:cssChanges.width});
                vendorPrefixes(this,'transform',translate(cssChanges.left, cssChanges.top));
                validateChangesAndComplete(tile);
            }
        }
        else {
            ...
            if (changed) {
                if(!Modernizr.csstransitions) {
                    ...
                    this.$el.css({height:cssChanges.height, width:cssChanges.width});
                    //vendorPrefixes(this,'transform','translate3d('+cssChanges.left+'px, '+cssChanges.top+'px, 0px)');
                    vendorPrefixes(this,'transform',translate(cssChanges.left, cssChanges.top));
                }
            }
